...
ZIP_FILENAME=couchbase-lite-c-enterprise-windows-x86_64.zip
cp "../bin/${ZIP_FILENAME}" .
...
cmake -DCMAKE_PREFIX_PATH=$CBL_LIB_DIR/libcblite-3.2.0 -DCMAKE_BUILD_TYPE=Release ..
cmake --build . --config Release
pushd Release > /dev/null
cp "$CBL_LIB_DIR/libcblite-3.2.0/bin/cblite.dll" .
./verify.exe
popd > /dev/null
popd > /dev/null
